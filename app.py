...
if _cloud_sql:
    # Cloud SQL via Unix socket (pg8000 driver, no binary dependency issues)
...
# Ensure tables exist; NEVER drops or renames anything.
Base.metadata.create_all(engine)

# Migrate: add set_number column if it doesn't exist (for existing PostgreSQL/SQLite DBs)
try:
    with engine.begin() as con:
        # Check if column exists
        try:
            con.execute(text("SELECT set_number FROM workout LIMIT 1"))
            log.info("Column set_number already exists")
        except Exception:
            con.execute(text("ALTER TABLE workout ADD COLUMN set_number INTEGER"))
